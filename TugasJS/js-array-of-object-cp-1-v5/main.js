function gameGrouping(games) {
    if (games.length < 2) {
        ...
        return games.reduce((usepreValues, usecurVelues) => {
            if (usepreValues?.[usecurVelues.genre]?.title) {
                return {
                    ...usepreValues,
                    [usecurVelues.genre]: {
                        title: [...usepreValues[usecurVelues.genre].title, usecurVelues.title]
                    }
                }
            } else {
                const res = {
